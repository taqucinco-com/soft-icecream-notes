...
2. `Task`ツールで`subagent_type: ui-verify-judge`を**同期的に（`run_in_background: false`を指定して）**呼び出す。7-A/7-Bで用意した画像パス、（7-Aの場合）Figmaの構造情報、比較対象の期待（Figmaまたは依頼文）を渡す。**バックグラウンド（非同期）呼び出しは絶対に使わないこと。** GitHub Actions等の非対話的なCIセッションには後続ターンが無く、非同期呼び出しの完了通知を受け取れる機会が無いため、判定結果を使えないままターンが終わってしまう（実際にこの事故が発生し、ループが一度も回らずに完了報告コメントも投稿されなかった）。
3. 応答のJSON中の`loop_verdict`で分岐する。
   ...
   - **`retry`かつイテレーションカウンタ < 10**: `criteria`の`mismatch`指摘をもとにコードを修正し、**呼び出し元skill自身のアプリのビルド・インストール・起動手順に従ってやり直し**、スクリーンショットを撮り直す（このskill=`flutter-ui-verify`では2節「アプリをビルド・起動する」。`flutter-ui-verify-ci`/`flutter-ui-ios-verify-ci`ではそれぞれの「アプリをビルド・インストール・起動する」節。`flutter-ui-ios-verify`では2節。それぞれのCI/ローカルの制約に従うこと。**`nohup flutter run`を使ってよいのはローカル版だけ**で、CI版はビルド→インストール→起動のコマンド列を使う）。カウンタを+1して7-Cの2.（`ui-verify-judge`の呼び出し）に戻る。

...
5. 画像・メタデータの両方を確認し、以下のチェックリストに沿って評価する。目視の印象だけで「だいたい合っている」と済ませない。
...
| 要素の有無 | ワイヤーフレームに描かれた個々の要素（タイトル、ボタン、チップ、カード、チャート等）が実装側にすべて存在するか。`get_metadata`のname一覧と`uiautomator dump`のtext/content-descを突き合わせ、過不足を具体的に列挙する |
| 配置・順序 | 各要素の相対的な位置関係（上下左右の並び順）が一致しているか。px単位の一致までは求めないが、`get_metadata`のx/y/width/heightと`uiautomator dump`のboundsを比較すると、目視より正確に判定できる |

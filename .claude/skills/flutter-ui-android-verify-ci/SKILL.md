...
- スクリーンショットおよび評価結果（`<name>-compare.md`/`<name>-compare.json`）の保存先は、`.claude/screenshots/<module>/`ではなく`flutter-android-operate-ci`skillの指示に従い`work/screenshots/`を使う（`.claude/`配下への書き込みはCIのサンドボックスで拒否されるため）。

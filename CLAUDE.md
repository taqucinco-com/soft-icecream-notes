...
- `claude-android.yaml`/`claude-ios.yaml`へ引き継いだ検証が完了した際は、GitHubのIssue/PRコメントに本来のスレッド返信機能が無いことを踏まえ、コメント本文の先頭に起動元コメントへの引用・リンク（`> 起動元コメントへの返信: <パーマリンク>`）を入れ、返信であることが分かる形で報告すること。

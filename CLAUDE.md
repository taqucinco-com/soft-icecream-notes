...
- `claude.yml`で依頼を処理する際は、依頼内容からiOS Simulatorでの確認が必要かどうかを判断すること。必要と判断した場合は `ios-verify-dispatch` スキルの手順に従い、`.github/workflows/claude-ios.yaml` をworkflow_dispatchで起動する。この判断はAndroid向けの`[ui-verify]`タグによる判定とは独立に行い、両方に該当する依頼であれば両方の検証を行う。
